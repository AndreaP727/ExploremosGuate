...
const climaDiv = document.getElementById("clima");
  ...
  const firebaseConfig = {
    ...
    storageBucket: "exploremos-guatemala.firebasestorage.app",
...
document.querySelectorAll('.destino').forEach(destino => {
  ...
  db.collection("puntuaciones").doc(destinoId).get().then(doc => {
    if (doc.exists) {
      votos = doc.data().votos;
      suma = doc.data().suma;
      actualizarInfo(stars, infoEl, votos, suma, userRating);
    } else {
      infoEl.innerText = "Aún sin votos";
    }
  });
  ...
  stars.forEach((star, index) => {
    star.addEventListener("click", () => {
      ...
      // Si el usuario ya votó, restamos su voto anterior
      if (userRating) {
        suma -= userRating;
        votos -= 1;
      }
      ...
      db.collection("puntuaciones").doc(destinoId).set({
        votos: votos,
        suma: suma
      });
